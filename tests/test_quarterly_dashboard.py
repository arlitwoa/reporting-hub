...
class QuarterlyDashboardTests(unittest.TestCase):
    ...
    def test_build_dashboard_html_contains_metrics(self):
        ...
        self.assertIn("Parent Initiative", html_doc)
        ...
        self.assertIn("Update the Jira saved filter", html_doc)
        self.assertIn("smart-current-quarter", html_doc)
        self.assertIn("issues/?filter=13880", html_doc)
        self.assertIn("Dates are derived from returned Epic Start/Due dates", html_doc)
        self.assertIn("Initiative Goal Target date", html_doc)
